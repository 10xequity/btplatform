...
**File:** docs/INDEX.md · **Version:** v1.0 · **Created:** 2026-08-02 · **Updated:** 2026-08-02
...
|---|---|---|---|
| `2026-08-02_handoff_v0_53_0.md` | v1.1 | 2026-08-02 | **State of record.** Delivery rule (§0), verified build state (§1), what shipped (§2), owner answers (§3), next build (§4), blockers (§5), doc hygiene (§7), next-session prompt (§8). |
| `2026-07-30_standards_v2_0.md` | v2.0 | 2026-07-30 | **Build and design law.** Trust order, versioning, DB rules, worker module pattern, design roster (§5), testing gates (§6), CI, member copy (§8), templating (§10). Reconstructed after a doc-set loss; §5/§6.5/§8/§10 anchors preserved so in-code citations stay valid. |
...
| `INDEX.md` | v1.0 | 2026-08-02 | This file. |
...
| `2026-08-01_uiux-review_admin-shell_v1_0.md` | v1.0 | 2026-08-01 | Touching any shell, rail, header, or contrast. Names the real AA offenders by file and line. Steps 1–3 of §6 shipped; **step 4 (SPA shell) is the last open item.** |
...
| `archive/2026-08-01_CHANGELOG-block_v0_45_0.md` | v1.0 | 2026-08-01 | **Superseded.** Handoff §7 directs deletion — this content now lives inside the reconstructed `CHANGELOG.md` that ships in the ZIP. Kept in the bundle purely so the v0.45.0 LFG detail is not lost in transit. Delete after confirming it against repo `CHANGELOG.md`. |
...
**C4 — `web/assets/build-status.js` v1.0 is stale and one entry is actively wrong.**
It tells testers the waiver gate at check-in is live. That gate was **removed in v0.33.1** (D-MIN-8,
owner: "no gating"). A tester reading it will file the absence of a block as a bug. Also: 5 rows
marked `soon` have shipped, and ~15 existing pages are missing from the PAGES registry.
**This is tester-facing copy and cheap to fix.** *Fix: build-status.js v1.1, first item in roadmap §7.*

**C5 — README §Roadmap is stale.** Items 1 and 2 shipped; the pointer still names handoff v0.52.0
and lists a blocker cleared in v0.52.0. *Fix: point it at `docs/2026-08-02_roadmap_v1_0.md`.*
...
*Changelog: v1.0 (2026-08-02) — created for the Claude Code port. Registers all ten documents with
status and read-trigger; opens the contradiction register with C1–C5.*
