...
|---|---|---|---|
| `2026-08-06_handoff_v0_98_0.md` | **v1.24** | 2026-08-06 | **State of record — END OF SESSION. READ THIS ONE.** **v0.98.0 shipped and verified live — §-1b W-E.2b staff card correction, and TRYOUTS IS FINISHED** (evaluate → roll up → place on a team; W-E.3 dropped forever, offers are an external system). Suite **1350**, files **83**, modules **51**, busters **400/64** at 0.98.0, ledger **0042** (read back live twice), **D-4 baseline 16 → 15 and the tryouts cluster is empty**. §2 carries the session's reusable finding — **a permissive server helper has one narrow input that behaves nothing like the wide one**: `parseList` JSON.parses a string before it splits, so a lone `"16"` becomes the number 16 and the list comes back EMPTY while `"14U, 16U"` works, so a form posting its raw comma box would delete data for one input in ten and pass review. Also: the pinned-spelling check paid off in the *other* direction (three flagged assertions never reddened, because the shape was chosen after reading them), and a re-rendering disclosure drops keyboard focus to `<body>`. §4 owner answers unchanged and both 2026-08-06 answers now delivered against. §6 promotes **W-F** (members/comms — standards §8 is load-bearing, a careless default there is a privacy defect). Supersedes and deletes v1.23 (v0.97.0). |
