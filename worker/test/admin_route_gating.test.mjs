...
/* ---------- pure primitives: MOVED to worker/testkit/route-extract.mjs (v0.103.0) ----------
   This file's header prescribed the move: "if a third consumer ever needs it, move it to
   worker/testkit/ and have both import it." §-1e priority 2's authorization matrix is that third
   consumer, and importing these from a TEST file would have re-registered this file's 12 tests
   inside it. The move is MECHANICAL — no logic changed — and THIS FILE IS THE VERIFIER: all seven
   tests below, four of them negative controls, still run against these exact functions, so any
   behavioural drift introduced by the move reddens the guard built to catch it. Re-exported so any
   existing importer keeps working. */
import {
  blankComments, lineOf, gateCallsIn, functionRanges, enclosing,
  statementStart, statementFrom, adminDispatchesIn, dispatchRegion,
  calleeNames, handlerGates,
} from "../testkit/route-extract.mjs";

export {
  blankComments, lineOf, gateCallsIn, functionRanges,
  statementStart, statementFrom, adminDispatchesIn, dispatchRegion,
};
