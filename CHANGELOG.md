...
**The owner's mobile + UX pass (asks 4 and 5, 2026-08-27).** The measured verdicts first: the internal-link audit walked all 1,565 hrefs across the 66-page corpus and found ZERO real dangling links (every apparent hit was the instrument's — `#roles`/`#views`/`#tournaments`/`#leagues` are hash view-routes their pages consume, and 404.html's absolute paths are correct for the Pages base); and most of the motion framework was already shipped (button press scale, strong ease tokens, modal popin, the M-4 home set). What was real and is now fixed: **five vw-unit widths** bled sideways under classic scrollbars — the login card overflowed index.html by 4px at phone width, measured live — all five (`.login-card`, `.ck-card`, `.kk-card`, `.theme-pop`, the waiver dialog) now size against their padded containers, and `mobile_tables.test.mjs` v1.1 gains a corpus-wide vw-width ratchet (watched red against all five, with a planted-rule positive control; font-size clamp()s exempt). Polish: the nav status chips rise from 9px to 10px (the smallest text in the product), the mobile tab scroller gets a thin scrollbar, and two occasional-frequency surfaces gain fenced entrances riding the shared tokens — the header menus (profile, appearance) pop in origin-aware from their trigger corner, and `say()` action notices ease in instead of snapping. Deliberately NOT animated, per the frequency framework: rail group toggles, the inline rename editor, and league board re-renders. The dense facility-calendar 10–11px data labels stay — idiomatic density, not a defect.

## v0.211.0 — 2026-08-27

**The league playoff — placement pools, the owner's "strength of power games" (his answer, 2026-08-27).** His format decision: tournaments end in a knockout bracket (already the tournament side's machinery); a league season ends in placement pools where each matchup "may be a match or 1 set or game depending on time." The wins-pods night IS that placement structure (rank-adjacent pods seeded by standings), so the delta is the FIXTURE: the pods path now honors `gamesPerMatch` 1–3 (3 = a full match, best of 3, per encounter; the level-capped path keeps RF-2B's cap of 2), where it silently ignored the select before. An encounter's games share a court back to back; `game_number` keeps play order; default 1 leaves every existing pods night byte-identical. The toolbar gains a pods-only "3 (full match)" option, the rounds select is honestly disabled for pods (the 3-slot pod round-robin is the night), and the format is named what the owner calls it. Guards: league_night v1.1 (+2, real-router 18-row full-match night + clamp/default net) and league_wins_pods v1.1 (+2, body-scoped UI pins), all watched red first. Playoff games stay ordinary league games, so standings, score links, the live board and print/email all work unchanged. Also recorded this session: A3 (rail collapse) resolved as already-shipped behavior — an explicit collapse wins, per the owner's "go with recommendation."

## v0.210.0 — 2026-08-27

**§-1c D-60 — one owner for the double-click rename.** The League board (v0.206.0) and Tournament Ops (v0.208.0) each carried a private `inlineRename`, and they had already drifted by one fix: the v0.207.0 focus-restore reached the League copy's Escape path only. The DOM/focus/latch mechanics now live once as `BT_ADMIN.inlineEdit(span, { onStart, commit })` in `admin-nav.js`; each page passes only its own commit (the PATCH, reload and error line). Unified on the stronger behaviour, so the League board gains focus-restore on a failed or no-op commit too. New guard `admin_inline_edit.test.mjs` pins the mechanics against the helper's own body, both call sites, and that neither page grows a second copy; the two page guards slim to what stays page-local (the PATCH shape, the roster wiring, the captain-is-never-sent pin). No route, schema or visual change.

## v0.209.0 — 2026-08-27

**§-1c D-59: a league week's match inserts land in ONE atomic batch.** `generateWeek` wrote each match with a sequential `await …run()` inside a nested loop — on BOTH the wins-pods branch and the level-capped path — so a league night (up to ~24 games) was ~24 D1 round-trips and, worse, a statement failing mid-loop left a PARTIAL week with no rollback. Both paths now accumulate their prepared statements and flush once with `env.DB.batch()` (PROMPT §3's atomicity primitive; the in-memory test DB batches atomically too), so a week is all-or-nothing. Row output is byte-identical — statements are pushed in the exact order they were run, and league_night.test.mjs / league_wins_pods.test.mjs (which drive POST /week through the real router and assert the inserted round/game_number/court/pairings) stay green. New guard league_week_batch.test.mjs (file 164, watched red first: 2 of 3 red) pins the shape — both paths batch, no per-match `.run()` survives in generateWeek — with a negative control. This came out of measuring Gemini's v0.206.0 review finding C1; recorded as D-59 that session rather than folded, because it is a whole-function refactor spanning both paths, not a wins-pods regression. Also struck the roadmap's §-1h M-1 (wide admin tables squash on mobile) — MEASURED as already shipped in v0.113.0 and guarded by mobile_tables.test.mjs, yet still listed as an open build candidate 95 releases later. Suite 2371 → 2374.

## v0.208.0 — 2026-08-26

**Owner ask: "add the double click to edit to the tournament page too."** The League board got double-click-to-rename in v0.206.0; this brings the same affordance to Tournament Ops (`tournament.js` / `tournament.html`). The tournament page had no team list — team names appeared only read-only in the pool grid, standings and byes, so fixing a pasted typo meant deleting the team and re-adding it. A compact roster now renders in the Teams panel (where teams are pasted): double-click (or Enter on) a name to edit it in place, Enter/blur commits via the existing `PATCH /api/admin/teams/:id { name }`, Escape cancels, and an empty or unchanged name never hits the server; on cancel/commit/error focus returns to the cell (`role=button`, `tabindex=0`) so a keyboard user keeps their place. A rename `refreshAll()`s, so the grid, standings and byes pick up the new name. The captain (T2-3) shows read-only beside each name for identification — it is a *contact* (`captain_contact_id`), not a team field, so editing it would rename a person across the org; the rename touches the team name only, exactly as on the League board. No new route, no schema change. Guard `tournament_inline_rename.test.mjs` (file 163), watched red first; a design pin asserts the rename never sends a captain field. This release also lands the interrupted v0.207.0 close docs (roadmap D-59 + handoff §1 Batch 11), and — the review API still unreachable (~4 days) — assembles the next manual Gemini Pro review block (admin-events.js + admin-manage.js + admin-schedule-editor.js). Suite 2362 → 2371.

## v0.207.0 — 2026-08-26

**Gemini review of v0.206.0's build, two findings folded (owner ran the paste-ready block through Gemini Pro; the API has been unreachable ~4 days).** The review covered the three files v0.206.0 shipped — admin-nav.js (rail collapse), admin-league.js (inline team rename), leagues_admin.js (wins-pods format). Measured finding by finding against the real source (most premises were reconstructions the diff did not show): (1) **KEPT — inline-rename a11y (B2):** cancelling a rename with Escape now returns focus to the team-name cell (`role=button`, `tabindex=0`) instead of dropping to `<body>`, so a keyboard user keeps their place. (2) **KEPT — 2-team pod (C4):** a lone 2-team wins-pods pod (only at N=2, or N=3 with the odd bye) now plays best-of-3 across all three game-slots rather than playing once and sitting idle for slots 2–3; its only opponent is each other, so the degenerate round-robin fills the night. Both watched red first (league_inline_rename.test.mjs +2, league_wins_pods.test.mjs +1). **REFUTED against the real code:** the rail collapse's `aside`/`layout` are null-guarded locals (not a global), every `.nav-group` carries a `data-key` by construction, and the active-group-collapse is a deliberate documented choice (explicit toggle wins); `data-team-name` holds the team *id* (not the name), and the `done` latch already blocks a double PATCH; the wins-pods sort can't produce NaN (the SQL COALESCEs wins/point_diff to 0), and court numbers are a cycling initial assignment resolved in the schedule editor — the level-capped path does the same and the owner declared it "not a defect." **RECORDED, not folded (§-1c):** converting generateWeek's per-match inserts to `env.DB.batch()` for atomicity — a real improvement, but the established level-capped path shares the exact sequential pattern, so it is a scoped refactor of the whole function, not a wins-pods regression. Suite 2359 → 2362.

## v0.206.0 — 2026-08-26

**Three owner asks built (2026-08-26).** (1) **Admin menu, Option A** — the rail's groups now default COLLAPSED except the one holding the current page, so the menu reads as ~6 headers you expand rather than 33 items at once ("get to ~10, subcategories ok"). The active group is read from the marked-active nav item (the open group and the highlight can't disagree); an explicit user toggle still wins either way (`bt_navgrp_<key>` = open/closed). admin_nav_collapse.test.mjs (file 160). (2) **Double-click to rename a team** — the QC generator's `EditableField` ported to the League Manager board: double-click (or Enter on) a team name to edit it in place, Enter/blur commits via the existing `PATCH /api/admin/teams/:id`, Escape cancels, and an empty or unchanged name never hits the server. league_inline_rename.test.mjs (file 161). (3) **The QC wins-ranked pods format, integrated** — a second `pairingMode` on the League Manager alongside the level-capped default: rank the ladder by wins (then differential, then name), cut it into rank-adjacent pods of 4 (a 6 absorbs the awkward remainder), and play a partial round-robin so every team gets exactly 3 distinct opponents that night, no repeats, no level cap. Exported `podSizes`/`pairWinsPods` are unit-tested directly (the 3-opponent guarantee, rank-adjacency, the pod-of-6 truncation, the odd-count bye); generateWeek branches to its own insert path (game_number = the pod slot) as an early return, leaving the level-capped path byte-identical. No schema change — it reads the standings/wins that already exist. A Format select ships on the League toolbar. league_wins_pods.test.mjs (file 162). Suite 2345 → 2359. Two guards caught their own regressions mid-build (an em dash in a new error string; a page-local `:focus-visible { outline:none }` that both tripped the focus-ring ratchet and would have killed the keyboard focus indicator — dropped).

## v0.205.0 — 2026-08-26

**§-1d admin profile menu + Settings off the rail (owner "Do all 3 as recommended", 2026-08-26).** The admin header gains a profile button (top-right ◉) opening a menu with **Account settings (→ settings.html) + Sign out** — the admin shell had neither before this: an admin could only sign out through the member shell or the rail's Settings link. The profile-menu CSS was PROMOTED from site-nav.js's injected styles to app.css (the one stylesheet both shells load) so admin and member share one definition, not two (D-23/D-24 class); guard profile_menu_css.test.mjs pins the single home. admin-nav.js gained logout wiring (new to the admin shell — POST /api/auth/logout, clear the local token, land on index.html) and the menu toggle (click / Escape / outside-click, aria-expanded), mirroring site-nav's contract. With the profile menu carrying Account settings, **Settings left the rail** — the last member page on it — so admin_rail_brevity now allows NO member-rail page at all (the settings.html exception dropped). The "Member site" rail group renamed **"Public site"** (its two items — Views & Embed, Calendar Feeds — configure public output, not the member site; the collapse-state key stays "site"). All three recommendations executed: Organization kept on the rail. Guards: admin_profile.test.mjs (file 158, markup + wiring + NCs), profile_menu_css.test.mjs (file 159, one home), header_shell v4.3 (the profile menu required in the admin canonical header, byte-identical across 40 pages), admin_rail_brevity v1.1 (no exceptions). The admin profile BUTTON ships visible (not hidden like the member one): admin pages bounce non-admins via guard() anyway, and a hidden .btn on admin-league (which has page-local .btn print rules) put hidden_overlay and shared_buttons in tension — shipping it visible sidesteps that cleanly; the menu and Sign out stay hidden until the token. Suite 2336 → 2345.

## v0.204.0 — 2026-08-26

**§-1d N-4 "League gets the same treatment": Tournament + League Management fold into one "Event Management" rail entry (owner "Agreed" 2026-08-26).** The two hash-carrying rail entries (admin-manage.html#tournaments / #leagues) become ONE hash-less "Event Management" item; the tournament/league choice moves onto the page as a quick-select tab row — the owner's "sub category under event management that can quick select those 2 options." The tabs reuse the shared `.tabs`/`.tab` component (N-4's reuse rule), are plain anchors to the existing #tournaments / #leagues scopes, so the switch rides the render path already there; admin-manage.js marks the active tab. The eight collapsed event tools' PARENT highlight map re-points from the two hashes to the single hash-less entry, and nav_highlight's full-href machinery is unchanged (its hash-parent NC inverted: the hash-less parent now matches, a stale hashed one would go dark). Removed from both rail sources (admin-nav.js NAV v2.27 + rail.partial.html) and propagated to all 40 admin pages via sync-rail; rail 35 → 34 items. Guards: admin_manage.test.mjs v1.2 (the single entry in both sources, the two old entries gone, the quick-select tabs, the active-tab marking) and nav_highlight.test.mjs v1.2 (the inverted parent NC) — the four fold assertions watched red first; one caught the marker_hygiene rule when a first draft used a character-distance window, rewritten to discrete behavioural tokens.

## v0.203.0 — 2026-08-26

**§-1d N-5, the admin-rail brevity pass (owner "Agreed" 2026-08-26).** With RF-4 (v0.184.0) having already collapsed the eight event tools into the manager hub, the measured remainder was the rail's "Member site" group: four of its six items (Home, Schedule Page, Live Scoreboard, Leagues Page) were pure outbound links to member pages — clicking one swapped the whole admin shell for the member site (the §-1c N-1c defect), and none ever received an active state, since those pages carry the MEMBER rail and the admin rail is not on screen there. Removed from both sources (admin-nav.js's NAV and rail.partial.html), propagated to all 40 admin pages via `sync-rail --write`. The two survivors (Views & Embed, Calendar Feeds) are admin config for public output and stay. The rail drops from 39 to 35 items. The forbid carries its exits, pinned: the admin header's "Member site" link and the Sandbox "View as member" both remain. New guard `admin_rail_brevity.test.mjs` (file 157, 8 tests, watched red first): the invariant is DERIVED — no admin-rail item may point at a page that ships the member rail (`site-nav.js`) — so re-adding any member page reddens, not just the four removed. It caught a fifth link the plan missed (`settings.html`, the shared personal-account page); that one is kept as the single documented exception (no admin-side equivalent — Organization is *org* settings) and pinned as the only allowed member page on the rail, with whether it too should leave raised as an open owner question.

## v0.202.0 — 2026-08-25

**§-1g C-2, the member half: the Play frame.** The signed-in rail's five Play items (Event Schedule, Leagues, Live scores, Community Play, Sub-Finder) are now tabs on ONE screen, `play.html`, that switch without reloading: each tab is the existing page in a same-origin chromeless iframe, created on first visit and kept (the v0.139.0 manager-hub idiom, reused not reinvented — every mechanism already existed: config.js's universal embed child, app.css's `body.embed` chrome hiding, site-nav's `?embed=1` skip). The rail's Play group is one item now, his 2026-08-08 sentence executed ("reducing the options on the left menu and keep items together that are only applicable to certain modules"); the five pages stay live at their own addresses, and a deep link to one still lights the Play rail item (`PLAY_PAGES`, site-nav v2.26). Sub-Finder's dedicated rail button (his 2026-08-22 word) became the frame's Sub-Finder tab — the name survives, the leagues/community top buttons are untouched, and the supersession is recorded at the pin (`sub_finder` v1.1). New guard `member_frame.test.mjs` (file 156, 13 tests, watched red first): the pane list, kept frames, hash routing, the embed contract, the rail collapse and its exit. Ratchets moved with reasons at each assertion: canonical member pages 18 → 19 (`header_shell`, `header_actions`, `org_contact`), rail item floors 9 → 5 / 15 → 13 (`member_nav_paint` v2.2).
...
RF-22's first press found the defect it exists to find: the live wipe/regenerate 500'd on a `FOREIGN KEY constraint failed`. Eighteen tables written by OTHER modules against seeded rows — `booking_spaces` held 33 live rows from facility bookings on test events — reference wiped tables but were never in the delete list, and the order guard only judged the tables already listed. The list now covers the whole graph (pass redemptions, passes, sales and sale items, subscriptions and grants, booking spaces and rental requests, form responses, families, consents, mutes, campaign sends, bans, field values, staff rates, SMS log, uploads), and `contacts ↔ families` turned out to be a genuine FK cycle no delete order can satisfy — one named UPDATE breaks it. wipe_order v1.1 adds a COMPLETENESS check that walks the full foreign-key graph transitively, so a module that grows a new reference to a wiped table reddens the suite instead of the live button.
...
**The cap is now ONE judgement with two readers, literally.** `BT_CAL` (`DAY_CAP` + `split()`) lives in `config.js` — the one script both shells load, `BT_SIGNUP_LINK`'s and `BT_THEME`'s precedent — and BOTH calendars render through `BT_CAL.split()`; neither file carries its own cap literal any more (the admin's local `CAL_DAY_CAP = 3` is deleted, deliberately). On the member grid, "+N more" expands the day **in place** (the member side has no modal; a second press collapses it, `aria-expanded` maintained), and a stale cached config.js without `BT_CAL` degrades to the uncapped pre-RF-7 cell for its ≤10-minute cache window rather than a dead calendar. The month pager and the List/Month mode switch now **refetch**, with the month window derived from `calCursor` (the 42 visible grid cells); List mode keeps its rolling window.

Guards: `events_calendar.test.mjs` v2.0 (7 red watched pre-fix, each accounted). The suite now **extracts `BT_CAL` from config.js's shipped bytes and executes it through the admin cell builder** — the judgement and the builder are tested as one artifact, never re-implemented — and `shippedCap()` enforces the one-judgement property itself (a cap literal reappearing in either reader fails). New member pins: `split()` called with the graceful-absence guard, the "+N more" control present, both pager buttons calling `load()`, and `load()` deriving its month window from `calCursor` — with NC-C1 (a pager wired back to `render()` fails) and NC-C2 (a planted second cap literal is seen by the scan). Suite 2123 → 2128.

## v0.175.0 — 2026-08-22

**§-1r RF-1(f) (owner 2026-08-18): the bracket press SAYS what happened — both defects behind "nothing happens", fixed at the button.** His item 1's sharpest finding, from the original measurement: every hop of the press was wired and the bracket really was written, yet the press read as silent — twice over. (1) **The second press was strictly silent in effect:** the engine answers 409 unless `replace: true`, and Tournament Ops never offered it — while `admin-brackets.js`'s `generate()` (and this same page's own `plCommit`) confirm with the server's sentence and re-POST with replace. That behaviour is now copied: on 409 the operator sees the engine's own words ("*<event> already has a bracket with N games…* Replace it? The current bracket is set aside, not lost."), and a yes re-POSTs with `replace: true` — through ONE `gen()` call site, so `bracket_rewire`'s one-writer uniqueness pin keeps its licence unmodified. (2) **The outcome used to render into `#warningsBox`, a whole grid ABOVE the button that was pressed** — right words, wrong place. It speaks at `#bracketNote` beside the button now (`aria-live="polite"`, so it is announced), and `#warningsBox` stays the schedule generator's voice.

Guards: `bracket_rewire.test.mjs` v1.1 (+4, watched red first) — the 409-confirm-replace behaviour pinned as a region verdict over the press handler, the at-the-button placement pinned in both directions (writes `#bracketNote`, does NOT write `#warningsBox`, and the element exists with its `aria-live`), plus NC-F1 (stripping the replace re-POST fails the verdict — its mutation targets the press's own `gen()` call, because a bare `replace: true` needle would hit the schedule generator's earlier confirm+replace first) and NC-F2 (pointing the outcome back above the fold fails the placement verdict). Suite 2119 → 2123.

RF-1's remaining halves stay queued as recorded: the print mode (admin.css/app.css carry zero `@media print`), button placement, BCC, and rendering the bracket on the Ops page itself.

## v0.174.0 — 2026-08-21

**§-1c D-53, found and closed (measuring Gemini's v0.173.0 review): NOTHING ON THE OWNER'S PATH EVER WROTE `ends_at` — so RF-4b's date rule would have read every real event as "active" forever, and his complaint would have quietly returned as real data grew.** Measured on live D1: every non-NULL `ends_at` belongs to sandbox seed events 90001–90006; the one real event carries NULL. Four layers each looked correct and jointly guaranteed NULL: the event modal never collected an end, `patchEvent`'s `allowed` list would have dropped it silently (D-34's "Saved." class), `createEvent`'s INSERT never named it, and `EVENT_FIELDS` stripped it before `insertEvent`'s `bag.ends_at` bind — which named it, as dead code. All four are fixed:

- The event modal collects **End date / End time** (optional; end time alone = same day; end date alone = end of that day; both blank stays NULL and the event stays visibly on the active list). A backwards end is refused in a sentence before the request is made.
- The recurring form collects **End time each night**, and `createRecurring` derives each instance's `ends_at` from its OWN date + that time-of-day (`endsForInstance`) — the base's datetime verbatim would stamp a first-night end on every later night. `editSeries` applies the same rule in SQL (`date(starts_at) || ' ' || end-time`) so a "this and future" edit can change when nights end without flattening their dates; a value with no time part is refused in a sentence.
- `patchEvent` allows `ends_at`; `createEvent` inserts it; `EVENT_FIELDS` carries it (bulk create rows persist it too).

**The Gemini review's own two recommendations, measured:** (1) its timezone "High Risk" claim is REFUTED on the data — all stored timestamps are uniform naive local (`YYYY-MM-DD HH:MM:SS`, zero `T`/`Z`/offsets across the corpus), the new writers keep that exact format (`${date} ${time}`, the same shape `starts_at` has always used), and `isPast` compares like-for-like strings; its "now captured once" speculation was already false (`nowStamp()` recomputes per render). The epoch rewrite was not taken — it would trade a measured-sound comparison for `Date`-parsing behaviour that varies by string shape. (2) Its ARIA recommendation is CONFIRMED and shipped: the Show-past toggle carries `aria-controls="mgPast"` and a live `aria-expanded`, maintained on every render and click.

Guards (`admin_manage.test.mjs` v1.1, all three watched red first): the whole write path pinned in one test — modal `bag()` sends `ends_at`, the modal ships the end-time input, `patchEvent`'s `allowed` carries it, `createEvent`'s INSERT names it, `EVENT_FIELDS` includes it; the per-instance derivation pinned (`endsForInstance` + the recurring form's input); the ARIA pair pinned on page and module. Suite 2116 → 2119.

## v0.173.0 — 2026-08-21

**§-1r RF-4 (option C) + RF-4b, with §-1c D-47 closed (owner 2026-08-18): the two management entry points.** His words, verbatim: *"tournaments and Leagues have their own buttons for management that sorts which they can pick of a filtered list. This also adds the caveate that as the list grows, the events that past must be removed from the active management list. They should still be available in the events page as an option to duplicate and historical data, but should not force a user to scroll through every event over time."* The admin rail gains **Tournament Management** and **League Management** (right after Events & Programs), both landing on the new `admin-manage.html` — one page, two hash scopes (`#tournaments` / `#leagues`), because hash-refined rail entries are the pattern the highlighter already speaks (`admin-users.html#roles` precedent) and the `hashchange` listener keeps the active mark correct when flipping between them without a reload. Each row opens that event's manager hub (`admin-manager.html?event=N` — WF-5). A hash-less deep visit defaults to `#tournaments` so the rail never sits dark.

**RF-4b's active/past rule is DATE-DERIVED, and the data chose it:** measured on live D1 before any code, 4 of 7 events carried `ends_at` in the past while `status` was still `published`/`in_progress`, and the only `completed` row was sandbox seed data — nothing in real use writes `completed`, so the obvious status filter would have shown every old event forever, which is the complaint itself. The shipped rule: past = ended by date, or the operator's explicit word (`completed`/`cancelled`); a NULL `ends_at` means not ended, so drafts under construction stay manageable. Past events sit behind a "Show past (N)" toggle and remain on Events & Programs for duplication and history — his own override, no new column, no status to remember. `GET /api/events`' staff projection gains `ends_at` (the column existed; the route never returned it — the check-the-writers class).

**The nine event-scoped rail entries do NOT collapse yet** — deliberately split per the standing scope rule: the pickers must exist and be verified live before nine pages lose their rail homes. The collapse (with its nine PARENT mappings) is the next unit.

**D-47 closed:** `admin-consent.html` had zero links from anywhere in `web/` — reachable only by typed URL since v0.25.0. Its home is now a line on the Waivers page ("Photo and video permissions live on Media consent"), where `nav_highlight` already parented it.

Guards: `admin_manage.test.mjs` (new, 7 tests, all watched red first): rail parity across BOTH sources (`rail.partial.html` and admin-nav.js's runtime NAV — the seam `sync-rail --write` never checks), the synced rail present on the new page itself, the default-hash rule, the picker→hub seam, the staff projection carrying `ends_at`, and **the active/past rule EXECUTED — extracted from the shipped file, never re-implemented — against the exact D1-measured trap** (ended-by-date + status still published), with a negative control that strips the date clause and proves the trap fixture exercises it. Suite 2109 → 2116; test files 138 → 139; sync-rail wrote all 40 admin pages.

**Also this session, no code: the owner chose Option 1 (merge as-is) on the Gemini review of v0.172.0, and its claims were measured anyway.** Edge case 1 CONFIRMED and recorded as D-52: the member header is sticky and `app.css` has no `scroll-margin` rules, so a fragment jump tucks the target heading under the header — he declined Option 2, so it is recorded, not built. Edge case 2 is true but bounded: a cached `home.html` with fresh `site-nav.js` misses the anchor for at most the ~10-minute HTML cache window, degrading to exactly the pre-fix behaviour, then self-heals. Option 3 (badge polling) ALREADY EXISTS — `fillNavBadges` has filled both counts since v0.105.0, and in a multi-page app every click is a fresh page load.

## v0.172.0 — 2026-08-20

**§-1r RF-12(4) + RF-12(2), with §-1c D-19 and D-50 (owner 2026-08-18): the member menu, his order.** His words: *"Realign that menu from most useful to least useful. Inbox should be 2 or 3, while Home at #1, then notifications (similar to logical for other apps). Group things together that make sense."* The signed-in rail is rebuilt most-useful-first into four groups: **You** (Home · Notifications · Inbox — his order verbatim), **Play** (Schedule · Leagues · Live scores · Community Play — Leagues above Live scores because a league night is a weekly destination and live scores an event-day one), **Explore** (Explore · Player Library), **Account** (My Profile · Membership · Settings · Help & FAQ). The grouping follows §-1f F-5/F-6's proposal, delivered v0.106.0 and held until his order arrived; F-5's signed-out/signed-in position stability is deliberately traded away — his explicit order outranks it.

**One assumption, load-bearing and stated: "Home at #1" is read as the member's OWN home — home.html, the screen titled "My Dashboard" — because his item 12 said "Dashboard should be the primary screen for members." If he corrects it, his word governs.** With that reading, the public card grid (index.html) keeps exactly ONE signed-in route, renamed **Explore** — his option B, RF-12(2) — which also ends **D-19** (two landing pages both named like home, indistinguishable in the rail since v0.106.0 recorded it).

**D-50 closed:** the rail's Notifications item pointed at `home.html#notifications` while no such id existed anywhere in `web/`, so the click silently landed at the top of the page. The feed box — where `loadNotificationsGroup()` actually renders in-app notifications — now carries the id (one reference in the whole corpus, renamed from `annBox`). The same sweep found a SECOND dead fragment, `index.html#signin` on the signed-out Sign in item: index.html has no such id (the login card is JS-rendered), so the fragment was dead weight and is dropped — the page IS the sign-in screen. Three more `#signin` fragments in page scripts (library.js, member-inbox.js, membership.js) are recorded as §-1c D-51 rather than fixed — they land on the login page as promised, so the fragment is inert, not a broken promise.

Guards: `member_nav_paint.test.mjs` v1.1 — his order pinned verbatim, exactly one Explore route to the grid, no two rail items sharing a name (D-19's class), and a **fragment contract**: every rail item that promises a page section must point at an id that exists in that page's static markup, with negative controls for a dead fragment and a demoted Notifications. All five new checks watched failing against the pre-fix rail — five reds, each accounted for, including NC-D50a red because the corpus then held TWO dead fragments. Suite 2104 → 2109.

**Also this session, no code: the owner-forwarded Gemini review of v0.171.0 was measured before choosing among its three post-merge options.** Verdicts: Option 1 (strict no-cache HTML) is IMPOSSIBLE on this hosting — GitHub Pages serves `Cache-Control: max-age=600` and offers no header control, and the near-equivalent already exists (network-first service worker, version-derived cache name, exact-match fallback — Block C, v0.101.0); Option 3's premise is REFUTED — the removed Manager tab never touched the server (`bt_login_role` has zero readers in `worker/src`), so a stale client shows a cosmetic tab that still signs in correctly and self-heals within the ~10-minute HTML cache window; Option 2 (migration toast) is WRONG-SCOPE — live D1 records one user holding the admin roles, so the entire staff population is the owner, who ordered the change. Decision: no new code; the existing machinery already does what the options aim at. The "trap state" edge case has a built-in exit — `bt_demo_member` is sessionStorage, so closing the tab ends the preview.
